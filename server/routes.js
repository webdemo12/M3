...
export async function registerRoutes(app) {
  ...
  app.post("/api/results", async (req, res) => {
    try {
      ...
      // Proper validation that handles 0 values
      if (!result_date || !time_slot || number_1 === null || number_1 === undefined || number_1 === '' || number_2 === null || number_2 === undefined || number_2 === '') {
        return res.status(400).json({ error: "All fields are required" });
      }

      // Validate ranges
      if (number_1 < 0 || number_1 > 999) {
        return res.status(400).json({ error: "Pati must be between 0 and 999" });
      }
      if (number_2 < 0 || number_2 > 9) {
        return res.status(400).json({ error: "Ghar must be between 0 and 9" });
      }
